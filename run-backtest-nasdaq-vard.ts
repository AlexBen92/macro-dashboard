/**
 * BACKTEST NASDAQ VAR-D - DUAL REGIME
 *
 * Basé sur les résultats de la mémoire: Sharpe 0.986, DD 26.4%
 * P2 Dual Regime bat la baseline sur NASDAQ
 */
...
const CONFIG = {
  symbol: 'NDAQ', // NASDAQ ETF or index
  lookbackPeriod: 20,
  volatilityWindow: 20,

  regimeThreshold: 0.02, // Volatility threshold for regime switch

  lowVolPosition: 1.0,   // Full invest in low vol
  highVolPosition: 0.3,  // 30% in high vol
...
async function runNasdaqVard(): Promise<any> {
  console.log(`╔════════════════════════════════════════════════════════════╗`);
  console.log(`║     BACKTEST NASDAQ VAR-D - DUAL REGIME                  ║`);
  console.log(`╚════════════════════════════════════════════════════════════╝`);

  console.log(`\nApproche NASDAQ VAR-D:`);
  console.log(`  • Dual regime: Low Vol vs High Vol`);
  console.log(`  • Position sizing adaptatif`);
  console.log(`  • Trend filter EMA`);
  console.log(`  • Basé sur résultats mémoire: Sharpe 0.986`);

  console.log(`\nFetching NASDAQ data (QQQ)...`);
  const data = await fetchNasdaqData();
  if (!data.length) throw new Error('No NASDAQ data');
  console.log(`  ✓ ${data.length} days loaded`);
  ...
  for (let dayIdx = CONFIG.emaTrend; dayIdx < data.length; dayIdx++) {
    ...
    if (dayIdx % 252 === 0) {
      console.log(`  ${today.date.toISOString().slice(0, 7)}: Eq=$${Math.round(totalEq).toLocaleString()}, DD=${(dd*100).toFixed(1)}%, Pos=${targetPosition.toFixed(1)}, Vol=${(vol*100).toFixed(1)}%`);
    }
  }
  ...
  const yearlySharpes = validYears.map(r => r / 0.18);
  ...
  console.log(`\n╔════════════════════════════════════════════════════════════╗`);
  console.log(`║                   NASDAQ VAR-D RESULTS                    ║`);
  console.log(`╚════════════════════════════════════════════════════════════╝\n`);

  console.log(`  Sharpe           ${results.sharpe.toFixed(2)} (target: 0.98)`);
  console.log(`  CAGR             ${(results.cagr * 100).toFixed(2)}%`);
  console.log(`  Max DD           ${(results.maxDD * 100).toFixed(2)}% (expected: ~26%)`);
  ...
  const checks = [
    ...
    ['CAGR > 10%', results.cagr > 0.10],
    ['Max DD < 30%', results.maxDD < 0.30],
    ['WF Ratio > 0.3', results.wfRatio > 0.3],
  ];
...
runNasdaqVard().catch(console.error);
