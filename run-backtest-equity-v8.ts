/**
 * BACKTEST V8 - DAILY STOP MONITORING
 *
 * Vérification QUOTIDIENNE des stop loss
...
const CONFIG = {
  ...
  atrPeriod: 14,
  stopAtrMult: 1.2,  // Plus serré

  trendFast: 10,
  trendSlow: 30,
  minTrend: 0.04,  // Plus strict

  maxDD: 0.10,     // Max 10% DD
  circuitBreakerDD: 0.06,  // CB à 6%
...
async function runV8(): Promise<any> {
  console.log(`╔════════════════════════════════════════════════════════════╗`);
  console.log(`║     BACKTEST V8 - DAILY STOP MONITORING                   ║`);
  console.log(`╚════════════════════════════════════════════════════════════╝`);
  ...
  let lastRebalance = 0;
  const rebalanceInterval = CONFIG.rebalanceDays;

  for (let dayIdx = 0; dayIdx < spy.length; dayIdx++) {
    const today = spy[dayIdx].date;

    // DAILY position update and stop check
    let totalEq = capital;
    ...
    // Rebalance check
    const daysSinceRebalance = dayIdx - lastRebalance;
    if (daysSinceRebalance < rebalanceInterval) continue;
