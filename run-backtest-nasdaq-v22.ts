/**
 * BACKTEST NASDAQ V22 - NO NEGATIVE YEARS
 *
 * Objectif: Éliminer les années négatives
 * - Filtre de marché plus strict
 * - Exit anticipée avant drawdowns
 */
...
const CONFIG = {
  ...
  lowVolThreshold: 0.018,  // Stricter threshold
  highVolThreshold: 0.025,

  lowVolPosition: 1.3,     // Slightly higher leverage
  midVolPosition: 0.5,
  highVolPosition: 0.2,
...
async function runNasdaqV22(): Promise<any> {
  console.log(`╔════════════════════════════════════════════════════════════╗`);
  console.log(`║     BACKTEST NASDAQ V22 - NO NEGATIVE YEARS               ║`);
  console.log(`╚════════════════════════════════════════════════════════════╝`);

  console.log(`\nObjectif V22:`);
  console.log(`  • Zéro année négative`);
  console.log(`  • Filtres plus stricts`);
  console.log(`  • Exit anticipée`);
  ...
  for (let dayIdx = CONFIG.emaTrend; dayIdx < data.length; dayIdx++) {
    ...
    // Early exit if year is going negative
    if (currentYearRet < -0.02 && shares > 0) {
    ...
    if (shares > 0 && entryPrice > 0) {
      ...
      // Exit if 5 consecutive down days
      if (consecutiveDownDays >= 5) {
        cash += shares * today.close;
        shares = 0;
        entryPrice = 0;
        lastRebalance = dayIdx;
    ...
    // Must be above EMA200
    if (today.close < ema200) {
      targetPosition = 0;
    }
    // EMA50 must be above EMA200 (golden cross)
    ...
    if (dayIdx % 252 === 0) {
      console.log(`  ${today.date.toISOString().slice(0, 7)}: Eq=$${Math.round(totalEq).toLocaleString()}, DD=${(dd*100).toFixed(1)}%, Pos=${targetPosition.toFixed(1)}x, YTD=${(currentYearRet*100).toFixed(1)}%`);
    }
  }
  ...
  const negativeYears = validYears.filter(r => r < 0).length;
  ...
  console.log(`\n╔════════════════════════════════════════════════════════════╗`);
  console.log(`║                   NASDAQ V22 RESULTS                       ║`);
  console.log(`╚════════════════════════════════════════════════════════════╝\n`);
  ...
  console.log(`  Negative Years   ${results.negativeYears} / ${validYears.length}`);
  ...
  const checks = [
    ...
    ['Negative Years = 0', results.negativeYears === 0],
    ['WF Ratio > 0.4', results.validation.wf_ratio > 0.4],
  ];
  ...
  console.log(`\n  VERDICT: ${passCount >= 5 ? '✅ VALIDÉ' : passCount >= 4 ? '🟡 ACCEPTABLE' : '❌ REJETÉ'} (${passCount}/6)`);

  return results;
}

runNasdaqV22().catch(console.error);
