/**
 * GET /api/ofi-obi-status
 *
 * Proxy-fetch VPS file (DASH_DATA_ORIGIN, fallback baked-in IP) — same as
 * /api/orderflow-status. Local-dev fallback reads /public/data.
 *
 * Staleness threshold 25h (registres statiques, cron daily).
 */
import { NextResponse } from 'next/server';
import { readFileSync, statSync, existsSync } from 'node:fs';
import { join } from 'node:path';

const LOCAL_FILE = join(process.cwd(), 'public', 'data', 'ofi_obi_status.json');
const DASH_DATA_ORIGIN = process.env.DASH_DATA_ORIGIN || 'http://187.124.38.41/dash-data';
const REMOTE_URL = DASH_DATA_ORIGIN
  ? `${DASH_DATA_ORIGIN.replace(/\/$/, '')}/ofi_obi_status.json`
  : null;
...
export async function GET() {
  ...
  try {
    if (!existsSync(LOCAL_FILE)) {
      return staleBody('ofi_obi_status.json not found', 503);
    }
