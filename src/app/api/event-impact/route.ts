 ...
 *
 * Macro event reaction stats (CPI/NFP/FOMC on ES=F). Daily export 05:53 UTC.
...
const LOCAL_FILE = join(process.cwd(), 'public', 'data', 'event_impact_status.json');
const DASH_DATA_ORIGIN = process.env.DASH_DATA_ORIGIN || 'http://187.124.38.41/dash-data';
const REMOTE_URL = DASH_DATA_ORIGIN
  ? `${DASH_DATA_ORIGIN.replace(/\/$/, '')}/event_impact_status.json`
  : null;
...
export async function GET() {
  ...
  try {
    if (!existsSync(LOCAL_FILE)) {
      return staleBody('event_impact_status.json not found', 503);
    }
