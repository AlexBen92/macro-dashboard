 ...
 * GET /api/corr-macro
 *
 * Extended crypto-macro correlation panel (BTC/ETH/ALTS_INDEX × equities,
 * indices, M2SL/UNRATE), computed on the VPS. NYSE-session alignment
 * (intersection of trading dates, no interpolation). Daily export 05:41 UTC.
 * Staleness threshold 26 hours. Contexte informatif — jamais un signal.
 */
import { NextResponse } from 'next/server';
import { readFileSync, statSync, existsSync } from 'node:fs';
import { join } from 'node:path';

const LOCAL_FILE = join(process.cwd(), 'public', 'data', 'corr_macro_status.json');
const DASH_DATA_ORIGIN = process.env.DASH_DATA_ORIGIN || 'http://187.124.38.41/dash-data';
const REMOTE_URL = DASH_DATA_ORIGIN
  ? `${DASH_DATA_ORIGIN.replace(/\/$/, '')}/corr_macro_status.json`
  : null;
...
function buildResponse(body: string, fallbackMtimeMs: number) {
  let parsed: { as_of?: string; last_export_success?: string } = {};
  try {
    parsed = JSON.parse(body) as { as_of?: string; last_export_success?: string };
  } catch {
    // ignore: staleness falls back to mtime
  }
  const exportMs = parsed.last_export_success
    ? Date.parse(parsed.last_export_success)
    : NaN;
  const asOfMs = parsed.as_of ? Date.parse(parsed.as_of) : NaN;
  const refMs = !Number.isNaN(exportMs)
    ? exportMs
    : !Number.isNaN(asOfMs)
      ? asOfMs
      : fallbackMtimeMs;
  const lastExportAgeMs = Date.now() - refMs;
...
export async function GET() {
  ...
  try {
    if (!existsSync(LOCAL_FILE)) {
      return staleBody('corr_macro_status.json not found', 503);
    }
