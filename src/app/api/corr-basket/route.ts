 ...
 * GET /api/corr-basket
 *
 * Liquid basket + edge futures correlation matrix (daily Pearson
 * log-returns, 60/120/252d), computed on the VPS from regime matrix
 * parquets. Daily export 05:57 UTC. Staleness threshold 26 hours.
 */
import { NextResponse } from 'next/server';
import { readFileSync, statSync, existsSync } from 'node:fs';
import { join } from 'node:path';

const LOCAL_FILE = join(process.cwd(), 'public', 'data', 'corr_basket_status.json');
const DASH_DATA_ORIGIN = process.env.DASH_DATA_ORIGIN || 'http://187.124.38.41/dash-data';
const REMOTE_URL = DASH_DATA_ORIGIN
  ? `${DASH_DATA_ORIGIN.replace(/\/$/, '')}/corr_basket_status.json`
  : null;
...
export async function GET() {
  ...
  try {
    if (!existsSync(LOCAL_FILE)) {
      return staleBody('corr_basket_status.json not found', 503);
    }
